...
define([
    ...
    return Class.extend({
        defaults: {
			...
			elementNameBackground : 'background_img',
        ...
        initialize: function (config) {
			...
			self. imgCntBackground =  $(self.updateSectionBackground).find('img').length + 1;
        ...
        _addButton:function(config) {
			...
			document.addEventListener(self.inputBgPrefix, function(event) {
                console.log(event.detail); // Prints "Example of an event"
                ...
                $(elementSelector).attr('data-url', self.url_background);
                // or .mage('loader', 'show')
                 var loader = $('[data-role="loader"]').loader();
                 self.loader = loader;
                    ...
                    dataType: 'json',
                        ...
                        if (data.result != undefined) {
                            if (data.result.url != undefined)  {
                                ...
                                var htmlSection = '<div class="saved_images_container_background" style="float: left"  align="center">'
                                    + '<input type="hidden" name="'+ self.elementNameBackground + '[' + self.imgCntBackground      + '][name]' + '"' + 'value="' + data.result.name + '"' + '/>'
                                    + '<input type="hidden" name="' + self.elementNameBackground + '[' + self.imgCntBackground      + '][file]' + '"' + 'value="' + data.result.file + '"' + '/>'
                                    + '<img class="saved_image_background" onclick="addBorderBackground(event)"' + 'src=' + '"' +imgSrc
                                    + '" />' +
                                    '<br> <button onclick="deleteBackgroundImage(event)" >' + 'Delete' + '</button>';

                                $(self.updateSectionBackground).append(htmlSection);

                                self. imgCntBackground++;
                            } else {
                                $(elementSelector).next('span').html('Error');
                                $(elementSelector).hide();
                            }
